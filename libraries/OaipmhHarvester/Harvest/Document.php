...
class OaipmhHarvester_Harvest_Document extends OaipmhHarvester_Harvest_Abstract
{
    ...
    protected function _getDataForRecord($record)
    {
        ...
        // TODO For an unknown reason, in some cases (see EAD plugin), the xml
        // needs to be reset to fetch the Dublin Core nodes.
        $record = simplexml_load_string($record->asXml());

        // Registers may be useless.
    ...
    /**
     * Convenient helper to create an oai record from a document record.
     *
     * @param SimpleXml $record The original oai record.
     * @param string $xmlpath
     * @return SimpleXML The new oai record.
     */
     public function createOaiRecordFromXmlDocument($record, $xmlpath)
     {
        ...
        $recordXml = simplexml_load_string($recordXml->record->asXml());

        // The data should looks like a harvested record, so a new root is set.
        $dom = new DOMDocument();
        $dom->formatOutput = true;
        // Create the oai record (the needed root element).
        $r = $dom->createElementNS('http://www.openarchives.org/OAI/2.0/', 'record');
        $dom->appendChild($r);
        // Append the oai record header (required for update).
        $h = dom_import_simplexml($record->header);
        $h = $dom->importNode($h, true);
        $h = $r->appendChild($h);
        // Create an intermediate oai metadata element.
        $m = $dom->createElement('metadata');
        $m = $r->appendChild($m);
        // Append the processed record inside the "metadata" element.
        $x = dom_import_simplexml($recordXml);
        $x = $dom->importNode($x, true);
        $x->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns', self::XML_NAMESPACE);
        $x->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:' . self::DC_PREFIX, self::DC_NAMESPACE);
        $x->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:' . self::DCTERMS_PREFIX, self::DCTERMS_NAMESPACE);
        $x->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:' . self::XSI_PREFIX, self::XSI_NAMESPACE);
        $x->setAttribute('xsi:schemaLocation', self::XML_NAMESPACE . ' ' . self::METADATA_SCHEMA);
        $x = $m->appendChild($x);

        // Finally, return it as simplexml.
